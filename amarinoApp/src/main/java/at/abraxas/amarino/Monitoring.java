...
public class Monitoring extends Activity implements LogListener, View.OnClickListener {
	...
	private void sendData(String address, String msg){
		...
		intent.putExtra(DefaultAmarinoServiceIntentConfig.EXTRA_FLAG, selectedFlag);		//选择模式
		intent.putExtra(DefaultAmarinoServiceIntentConfig.EXTRA_DATA_TYPE, DefaultAmarinoServiceIntentConfig.STRING_EXTRA); //数据类型
		intent.putExtra(DefaultAmarinoServiceIntentConfig.EXTRA_DATA, msg);	//具体数据
		Monitoring.this.startService(intent);
	}
	
	
	//通过handler 发送一个runnable 线程过去  可以直接设置VIEW
	@Override
