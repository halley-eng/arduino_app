...
public class AmarinoDbAdapter {
	...
	private static class DatabaseHelper extends SQLiteOpenHelper {
		...
		public DatabaseHelper(Context context) {
			super(context, DefaultDBConfig.DATABASE_NAME, null,
					DefaultDBConfig.DATABASE_VERSION);
			config = new DefaultDBConfig();
		}

		public DatabaseHelper(Context context, DBConfig config) {
		...
		public void onCreate(SQLiteDatabase db) {
			...
			db.execSQL("CREATE TABLE " + getConfig().getDevicesTableName()
					+ " (" + KEY_DEVICE_ID + " INTEGER PRIMARY KEY,"
					+ KEY_DEVICE_ADDRESS + " TEXT UNIQUE," + KEY_DEVICE_NAME
					+ " TEXT" + ");");

			db.execSQL("CREATE TABLE " + getConfig().getEventsTableName()
					+ " (" + KEY_EVENT_ID + " INTEGER PRIMARY KEY,"
					+ KEY_EVENT_NAME + " TEXT NOT NULL," + KEY_EVENT_DESC
					+ " TEXT," + KEY_EVENT_VISUALIZER + " INTEGER,"
					+ KEY_EVENT_VISUALIZER_MIN + " NUMBER,"
					+ KEY_EVENT_VISUALIZER_MAX + " NUMBER," + KEY_EVENT_FLAG
					+ " INTEGER NOT NULL," + KEY_EVENT_PACKAGE_NAME
					+ " TEXT NOT NULL," + KEY_EVENT_EDIT_CLASS_NAME
					+ " TEXT NOT NULL," + KEY_EVENT_SERVICE_CLASS_NAME
					+ " TEXT NOT NULL," + KEY_EVENT_PLUGIN_ID
					+ " INTEGER NOT NULL," + KEY_EVENT_DEVICE_ID
					+ " INTEGER REFERENCES "
					+ getConfig().getDevicesTableName() + "(_id) " + ");");
		}

		@Override
		public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
			// TODO we sould provide a data migration method instead
			Log.w(TAG, "Upgrading database from version " + oldVersion + " to "
					+ newVersion + ", which will destroy all old data");
			db.execSQL("DROP TABLE IF EXISTS "
					+ getConfig().getDevicesTableName());
			db.execSQL("DROP TABLE IF EXISTS "
					+ getConfig().getEventsTableName());
			onCreate(db);
	...
	public AmarinoDbAdapter(Context ctx, DBConfig config) {
		this.mCtx = ctx;
		mDbHelper = new DatabaseHelper(mCtx);
		mDbHelper.setConfig(config);
	}

	/**
	 * Open the database with the {@link DefaultDBConfig} configuration. If it
	 * cannot be opened, try to create a new instance of the database. If it
	 * cannot be created, throw an exception to signal the failure
	 * 
	 * @param ctx
	 *            the working Context for this class
	 ...
	 * successfully created return the new rowId for that device, otherwise
	...
	public boolean deleteDevice(long deviceId) {
		...
		if (DEBUG)
			Logger.d(TAG, "delete device with id " + deviceId + ": "
					+ numEvents + " associated events removed");
		return mDb.delete(mDbHelper.getConfig().getDevicesTableName(),
				KEY_DEVICE_ID + "=" + deviceId, null) > 0;
	}

	public BTDevice getDevice(String address) {
		BTDevice device = null;
		Cursor c = mDb.query(mDbHelper.getConfig().getDevicesTableName(), null,
				KEY_DEVICE_ADDRESS + " like ?", new String[] { address }, null,
				null, null);
		...
		if (c.moveToFirst()) {
			do {
				String address = c.getString(c
						.getColumnIndex(KEY_DEVICE_ADDRESS));
				String name = c.getString(c.getColumnIndex(KEY_DEVICE_NAME));
				long id = c.getLong(c.getColumnIndex(KEY_DEVICE_ID));
	 ...
	 * successfully created return the new rowId for that event, otherwise
	 * return a -1 to indicate failure.
	 * 
	 * @param address
	 *            the address of the device
	...
	public long createEvent(Event event) {
		...
		return mDb.insert(mDbHelper.getConfig().getEventsTableName(), null,
				initialValues);
	}
	...
	public boolean deleteEvent(long rowId) {
		return mDb.delete(mDbHelper.getConfig().getEventsTableName(),
				KEY_EVENT_ID + "=" + rowId, null) > 0;
	}
	...
	public int deleteEvents(long deviceId) {
		return mDb.delete(mDbHelper.getConfig().getEventsTableName(),
				KEY_EVENT_DEVICE_ID + "=" + deviceId, null);
	}

	public Event getEvent(long deviceId, int pluginId) {
		Event e = null;
		Cursor c = mDb.query(
				mDbHelper.getConfig().getEventsTableName(),
				null,
				KEY_EVENT_DEVICE_ID + "=? AND " + KEY_EVENT_PLUGIN_ID + "=?",
				new String[] { String.valueOf(deviceId),
						String.valueOf(pluginId) }, null, null, null);

		if (c == null) {
			if (DEBUG)
				Logger.d(TAG, "no event found for device with id: " + deviceId
		...
		if (c.moveToFirst()) {
			...
			float minVal = c.getFloat(c
					.getColumnIndex(KEY_EVENT_VISUALIZER_MIN));
			float maxVal = c.getFloat(c
					.getColumnIndex(KEY_EVENT_VISUALIZER_MAX));
			char flag = (char) c.getInt(c.getColumnIndex(KEY_EVENT_FLAG));
			String packageName = c.getString(c
					.getColumnIndex(KEY_EVENT_PACKAGE_NAME));
			String editClassName = c.getString(c
					.getColumnIndex(KEY_EVENT_EDIT_CLASS_NAME));
			String serviceClassName = c.getString(c
					.getColumnIndex(KEY_EVENT_SERVICE_CLASS_NAME));

			e = new Event(id, name, desc, visualizer, flag, packageName,
					editClassName, serviceClassName, pluginId, deviceId);
	...
	public ArrayList<Event> fetchEvents(long deviceId) {
		...
		Cursor c = mDb.query(mDbHelper.getConfig().getEventsTableName(), null,
				KEY_EVENT_DEVICE_ID + "=" + deviceId, null, null, null, null);

		if (c == null) {
			if (DEBUG)
				Logger.d(TAG, "no events found for device with id: " + deviceId);
			return events;
		}
		if (c.moveToFirst()) {
			do {
				...
				String name = c.getString(c.getColumnIndex(KEY_EVENT_NAME));
				String desc = c.getString(c.getColumnIndex(KEY_EVENT_DESC));
				int visualizer = c.getInt(c
						.getColumnIndex(KEY_EVENT_VISUALIZER));
				float minVal = c.getFloat(c
						.getColumnIndex(KEY_EVENT_VISUALIZER_MIN));
				float maxVal = c.getFloat(c
						.getColumnIndex(KEY_EVENT_VISUALIZER_MAX));
				...
				Event e = new Event(id, name, desc, visualizer, flag,
						packageName, editClassName, serviceClassName, pluginId,
						deviceId);
				e.visualizerMinValue = minVal;
				e.visualizerMaxValue = maxVal;
				...
				events.add(e);
				if (DEBUG)
					Logger.d(TAG, "event found: " + e.name + " - id="
							+ e.pluginId);
			} while (c.moveToNext());
		} else {
			if (DEBUG)
	...
	public int updateEvent(Event event) {
		...
		return mDb.update(mDbHelper.getConfig().getEventsTableName(), values,
				KEY_EVENT_ID + "=" + event.id, null);
	}
